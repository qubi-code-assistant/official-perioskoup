...
// A08: ariaHidden prop — pass true when the SVG is inside a labelled link/container.
// When ariaHidden is false (standalone use), a <title> provides the accessible name.
...
// LogoMark inside LogoFull is decorative — the wordmark text provides the name.
export function LogoFull({ height = 28, color = '#C0E57A', className = '' }: LogoProps) {
  ...
  return (
    <div className={className} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
      <LogoMark height={iconSize} color={color} ariaHidden={true} />
      <span style={{
...
export function LogoLime({ height = 32, className = '' }: { height?: number; className?: string }) {
  return (
    <div className={className} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
      <LogoMark height={height} color="#C0E57A" ariaHidden={true} />
      <span style={{
        fontFamily: 'Gabarito, sans-serif',
        fontWeight: 700,
        fontSize: `${Math.round(height * 0.6)}px`,
